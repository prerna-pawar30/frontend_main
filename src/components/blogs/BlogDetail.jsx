...
const CSS = `
  ...
  :root{--accent:#E68736;--accent-hover:#D4751F;--accent-light:#FFF4EA;--accent-mid:#F5A55A;--bg:#FAFAF8;--white:#FFFFFF;--text:#0E0E0C;--text-2:#3A3A36;--text-3:#888880;--border:#E6E4DE;--border-soft:#F0EDE8;}
  *,*::before,*::after{box-sizing:border-box;} body{background:var(--bg);margin:0;}
  .f-display{font-family:'Playfair Display',Georgia,serif;} .f-body{font-family:'Outfit',system-ui,sans-serif;}
  .progress{position:fixed;top:0;left:0;right:0;z-index:100;height:3px;background:var(--accent);transform-origin:left;}
  .hero{background:linear-gradient(135deg,#0f172a 0%,#1e293b 50%,#0f172a 100%);position:relative;overflow:hidden;border-radius:20px;margin:80px 24px 0;}
  ...
  .back-btn:hover{color:#fff;} .back-btn:hover .back-icon{border-color:var(--accent);background:rgba(230,135,54,0.15);}
  ...
  .prose h2{font-size:1.65rem;padding-bottom:0.5rem;border-bottom:2px solid var(--border-soft);} .prose h3{font-size:1.25rem;}
  .prose p{margin-bottom:1.35em;} .prose a{color:var(--accent);text-underline-offset:3px;} .prose strong{color:var(--text);font-weight:600;}
  .prose ul,.prose ol{padding-left:1.5em;margin-bottom:1.35em;} .prose li{margin-bottom:0.4em;}
  ...
  .prose td{border-top:1px solid var(--border-soft);padding:0.75rem 1rem;color:var(--text-2);} .prose tr:hover td{background:#FDFBF8;}
  .prose hr{border:none;border-top:1px solid var(--border);margin:2.5em 0;}
  .tag{display:inline-flex;align-items:center;padding:5px 14px;border-radius:999px;background:var(--white);border:1.5px solid var(--border);font-size:12px;font-weight:500;color:var(--text-3);transition:all 0.2s;cursor:pointer;font-family:'Outfit',sans-serif;}
  ...
  .field::placeholder{color:#C0BEB8;} .field:focus{border-color:var(--accent);box-shadow:0 0 0 3px #E6873318;background:var(--white);}
  ...
  .submit-btn:active:not(:disabled){transform:translateY(0);} .submit-btn:disabled{background:#D0CFCA;box-shadow:none;cursor:not-allowed;}
  .info-row{display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--border-soft);} .info-row:last-child{border-bottom:none;padding-bottom:0;}
  ...
  .divider{display:flex;align-items:center;gap:12px;} .divider::before,.divider::after{content:'';flex:1;height:1px;background:var(--border);}
  @keyframes spin{to{transform:rotate(360deg)}} .spinner{width:17px;height:17px;border-radius:50%;border:2px solid rgba(255,255,255,0.35);border-top-color:white;animation:spin 0.7s linear infinite;}
  ::-webkit-scrollbar{width:5px;} ::-webkit-scrollbar-track{background:var(--bg);} ::-webkit-scrollbar-thumb{background:#D4D2CC;border-radius:4px;}
  @media(max-width:1024px){.content-grid{grid-template-columns:1fr !important;} .content-grid>*:last-child{position:static !important;}}
  @media(max-width:640px){.hero{margin:70px 12px 0;border-radius:14px;} .hero-inner{padding:32px 24px 32px !important;} .grid-inner{padding:32px 20px 60px !important;} .feat-wrap{padding:24px 20px 0 !important;}}
`;

const MetaChip = ({ d, children }) => (
  <span className="meta-chip">
    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d={d} /></svg>
    {children}
  </span>
);
    ...
    <>
      ...
      <div className="f-body" style={{ minHeight:'100vh', background:'var(--bg)' }}>
        ...
        <motion.section className="hero" initial="hidden" animate="show" variants={stagger}>
          <div className="hero-dots" /><div className="hero-bar" />
          <div className="hero-inner" style={{ maxWidth:1240, margin:'0 auto', padding:'44px 40px' }}>
            ...
            <motion.h1 variants={fadeUp} className="f-display"
            ...
            {(blog.shortDescription || blog.description) && (
              <motion.p variants={fadeUp} style={{ fontSize:'1.0625rem', color:'rgba(255,255,255,0.6)', maxWidth:640, lineHeight:1.7, margin:'0 0 32px' }}>
                {blog.shortDescription || blog.description}
              </motion.p>
            )}
            ...
            <motion.div variants={fadeUp} style={{ display:'flex', flexWrap:'wrap', alignItems:'center', gap:14 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <div style={{ width:40, height:40, borderRadius:'50%', background:'rgba(230,135,54,0.2)', color:'#E68736', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:15, border:'2px solid rgba(230,135,54,0.35)' }}>
                  {(blog.author || 'D').charAt(0)}
                </div>
                <div>
                  <p style={{ margin:0, fontWeight:600, fontSize:13, color:'#fff' }}>{blog.author || 'Digident Team'}</p>
                  <p style={{ margin:0, fontSize:12, color:'rgba(255,255,255,0.5)' }}>{publishDate}</p>
                </div>
              </div>
              ...
              {comments.length > 0 && <><span className="meta-divider" /><MetaChip d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z">{comments.length} comments</MetaChip></>}
              {blog.readTime && <><span className="meta-divider" /><MetaChip d="M12 2a10 10 0 1 0 0 20A10 10 0 0 0 12 2z M12 6v6l4 2">{blog.readTime} min read</MetaChip></>}
            </motion.div>
          </div>
        </motion.section>
        ...
        {(blog.bannerImage || blog.featuredImage) && (
          <motion.div className="feat-wrap" initial={{ opacity:0, y:18 }} animate={{ opacity:1, y:0 }}
          ...
          <div className="content-grid" style={{ display:'grid', gridTemplateColumns:'1fr 420px', gap:'52px', alignItems:'start' }}>
            ...
            <motion.div initial="hidden" whileInView="show" viewport={{ once:true, margin:'-40px' }} variants={stagger}>
              <motion.div variants={fadeUp}>
                <div className="prose">
                  <ReactMarkdown remarkPlugins={[remarkGfm]}>{blog.contentMarkdown || blog.content}</ReactMarkdown>
                </div>
              </motion.div>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
                ...
                <div className="sidebar-card">
                  ...
                  <div style={{ padding:'28px 28px 32px' }}>
                    <h3 className="f-display" style={{ fontSize:'1.5rem', fontWeight:700, color:'var(--text)', margin:'0 0 6px' }}>Join the discussion</h3>
                    <p style={{ fontSize:13, color:'var(--text-3)', margin:'0 0 24px' }}>Share your thoughts on this article.</p>
                    <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:14 }}>
                      ...
                      <textarea rows={5} placeholder="Your thoughts… *" required className="field" style={{ resize:'none' }}
                        value={comment.review} onChange={e => setComment({ ...comment, review:e.target.value })} />
                      <button type="submit" disabled={submitting} className="submit-btn">
                        <AnimatePresence mode="wait">
                          {submitting
                            ? <motion.span key="s" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} style={{display:'flex',alignItems:'center',gap:8}}><span className="spinner"/>Sending…</motion.span>
                            : <motion.span key="p" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} style={{display:'flex',alignItems:'center',gap:8}}>
                                Post Comment
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
                              </motion.span>
                          }
                        </AnimatePresence>
                      </button>
                ...
                <div className="sidebar-card">
                  <div style={{ padding:'22px 24px' }}>
                    <p style={{ fontSize:11, fontWeight:700, letterSpacing:'0.14em', textTransform:'uppercase', color:'var(--text-3)', margin:'0 0 6px' }}>Article Details</p>
